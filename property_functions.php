...
if( !function_exists('houzez_submit_listing') ) {
    function houzez_submit_listing($new_property) {
        ...
        if( $prop_id > 0 ) {
            ...
            // Add Fully Furnished meta
            if( isset( $_POST['prop_furniture'] ) && ( sanitize_text_field( $_POST['prop_furniture'] ) == "on" ) ) {
                update_post_meta( $prop_id, 'fave_property_furniture',  1);
            }
            else{
                update_post_meta( $prop_id, 'fave_property_furniture',  0);
            }

            

            // Add Utilities Included meta
            if( isset( $_POST['prop_utilities'] ) && ( sanitize_text_field( $_POST['prop_utilities'] ) == "on" ) ) {
                update_post_meta( $prop_id, 'fave_property_utilities',  1);
            }
            else{
                update_post_meta( $prop_id, 'fave_property_utilities',  0);
            }

            // Add Pet Friendly meta
            if( isset( $_POST['prop_pet_friendly'] ) && ( sanitize_text_field( $_POST['prop_pet_friendly'] ) == "on" ) ) {
                update_post_meta( $prop_id, 'fave_property_pet_friendly',  1);
            }
            else{
            ...
            if( isset( $_POST['prop_bath_type'] ) ) {
                update_post_meta( $prop_id, 'fave_property_bath_type', sanitize_text_field( $_POST['prop_bath_type'] ) );
            }

            // Add Entrance Type post meta
            if( isset( $_POST['prop_entrance_type'] ) ) {
                update_post_meta( $prop_id, 'fave_property_entrance_type', sanitize_text_field( $_POST['prop_entrance_type'] ) );
            }
